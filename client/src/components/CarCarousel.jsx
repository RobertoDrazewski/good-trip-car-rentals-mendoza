...
export default function CarCarousel() {
  ...
  return (
    /* 📱 RESPONSIVE: gap reducido en móvil (gap-6) y optimizado en escritorio (md:gap-12) */
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-12 p-4 sm:p-6 max-w-7xl mx-auto py-12 md:py-20">
      {autos.map((auto) => (
        <div 
          key={auto.id} 
          /* 📱 RESPONSIVE: Bordes menos toscos en celular (rounded-[2rem] a md:rounded-[3.5rem]) */
          className="group bg-white rounded-[2rem] md:rounded-[3.5rem] border border-slate-50 shadow-[0_30px_60px_-15px_rgba(0,0,0,0.08)] overflow-hidden hover:shadow-[0_40px_80px_-20px_rgba(0,0,0,0.15)] transition-all duration-500 flex flex-col relative"
        >
          {/* BADGE DE VALORACIÓN */}
          ...
          <div className="h-64 sm:h-96 bg-slate-50 relative overflow-hidden flex items-center justify-center p-6 sm:p-12">
            ...
            <div className="absolute bottom-4 right-6 sm:bottom-6 sm:right-10 opacity-30 group-hover:opacity-100 transition-opacity">
          ...
          <div className="p-6 sm:p-10 md:p-14 flex-1 flex flex-col">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start gap-4 sm:gap-6 mb-8 sm:mb-10">
              <div className="flex-1 w-full">
                {/* 📱 RESPONSIVE: text-3xl en celu para que entre la patente y el modelo sin solaparse */}
              ...
              {/* 📱 PRECIOS VISIBLES: Caja alineada y destacada en celulares */}
            ...
            {/* GRILLA TÉCNICA - Reducido el gap vertical en móvil (gap-y-6) */}
            <div className="grid grid-cols-2 gap-x-4 gap-y-6 sm:gap-y-10 pt-8 sm:pt-10 border-t border-slate-100">
              <div className="flex items-center gap-3 sm:gap-5">
                <div className="p-2.5 sm:p-3.5 bg-slate-50 rounded-xl sm:rounded-2xl text-slate-900 group-hover:bg-yellow-500/10 transition-colors">
