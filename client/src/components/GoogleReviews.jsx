...
const RatingStars = ({ size = 14 }) => (
  <div className="flex text-yellow-500 gap-0.5 sm:gap-1" aria-label="5 estrellas de calificación">
    {[1, 2, 3, 4, 5].map((star) => (
...
/**
 * Icono de Instagram manual seguro
 */
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-12 w-full">
        ...
        <div className="flex flex-col lg:flex-row lg:items-end justify-between mb-10 md:mb-20 gap-6 md:gap-12 w-full">
          <div className="max-w-2xl text-left w-full">
            <p className="text-yellow-600 font-black uppercase text-[8px] sm:text-[10px] tracking-[0.25em] sm:tracking-[0.4em] mb-2 sm:mb-4 italic">
              {t('reviews_badge_tag')}
            </p>
            {/* 🛠️ SOLUCIONADO: Reducido text-4xl a text-3xl en móviles y controlado el corte de palabras */}
            <h2 className="text-3xl sm:text-6xl md:text-8xl font-black tracking-tighter text-slate-900 leading-none uppercase break-words">
          ...
          {/* 🛠️ SOLUCIONADO: Ajustado flex-row dinámico con gap estrecho en móviles para evitar colisiones */}
          <div className="bg-slate-900 p-5 sm:p-8 rounded-[1.8rem] sm:rounded-[3rem] shadow-xl flex items-center gap-4 sm:gap-8 border border-white/10 relative overflow-hidden group w-full lg:w-auto flex-shrink-0">
            ...
            <div className="text-center relative z-10 flex-shrink-0">
              <p className="text-4xl sm:text-6xl font-black text-white leading-none tracking-tighter">5.0</p>
              <div className="mt-1 flex justify-center"><RatingStars size={10} /></div>
            </div>
            
            <div className="h-10 sm:h-16 w-[1px] bg-white/10 flex-shrink-0" />
            
            <div className="flex flex-col gap-1 sm:gap-3 relative z-10 w-full sm:w-auto">
              <p className="text-white font-black text-[10px] sm:text-sm uppercase tracking-widest leading-tight whitespace-nowrap">
                Google Verified
              </p>
              <a href={GOOGLE_MAPS_LINK} target="_blank" rel="noreferrer" className="flex items-center gap-1 text-yellow-500 hover:text-white transition-colors w-fit touch-manipulation">
        ...
        {/* GRID DE RESEÑAS / SCROLL HORIZONTAL */}
        {/* 🛠️ SOLUCIONADO: p-5 en móvil para ganar espacio y control anti-desborde tipográfico */}
        <div className="flex overflow-x-auto pb-6 gap-5 snap-x snap-mandatory scrollbar-none md:grid md:grid-cols-3 md:gap-10 md:mb-20 md:pb-0 w-full">
          {reviews.map((rev) => (
            <div 
              key={rev.id} 
              className="bg-slate-50 p-5 sm:p-8 md:p-12 rounded-[1.8rem] md:rounded-[3.5rem] border border-transparent hover:border-slate-200 hover:bg-white hover:shadow-[0_40px_80px_-20px_rgba(0,0,0,0.1)] transition-all duration-500 group flex-shrink-0 w-[82vw] sm:w-[340px] md:w-auto snap-center flex flex-col justify-between overflow-hidden"
            >
              <div>
                <div className="flex items-center gap-3 sm:gap-5 mb-4 sm:mb-6 w-full">
                  <div className="w-10 h-10 sm:w-14 sm:h-14 bg-slate-900 rounded-full flex items-center justify-center font-black text-white text-base sm:text-lg shadow-lg group-hover:bg-yellow-500 group-hover:text-slate-900 transition-colors flex-shrink-0">
                    {rev.img}
                  </div>
                  <div className="min-w-0 flex-1">
                    {/* truncate evita el desborde si el nombre del cliente es kilométrico */}
                    <h4 className="font-black text-slate-900 uppercase text-xs sm:text-sm tracking-tight truncate">
                      {rev.name}
                    </h4>
                ...
                {/* 🛠️ SOLUCIONADO: text-sm en móvil para perfecto encuadre, line-clamp elástico */}
        ...
        {/* 🛠️ SOLUCIONADO: tracking controlado y padding interno elástico (px-4) con text-center */}
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4 sm:gap-6 mt-4 md:mt-0 w-full">
          <a 
            href={GOOGLE_MAPS_LINK}
            target="_blank" 
            rel="noreferrer" 
            className="group flex items-center justify-center gap-3 bg-slate-900 text-white w-full sm:w-auto px-4 sm:px-12 py-4 sm:py-6 rounded-xl sm:rounded-[2.5rem] font-black uppercase text-[10px] sm:text-xs tracking-[0.1em] sm:tracking-[0.25em] hover:bg-yellow-500 hover:text-slate-950 transition-all shadow-xl hover:-translate-y-1 active:scale-95 text-center touch-manipulation whitespace-normal min-h-[52px]"
          >
          ...
          <a 
            ...
            target="_blank" 
            rel="noreferrer" 
            className="group flex items-center justify-center gap-3 bg-white border-2 border-slate-900 text-slate-900 w-full sm:w-auto px-4 sm:px-12 py-4 sm:py-6 rounded-xl sm:rounded-[2.5rem] font-black uppercase text-[10px] sm:text-xs tracking-[0.1em] sm:tracking-[0.25em] hover:bg-slate-900 hover:text-white transition-all shadow-md hover:-translate-y-1 active:scale-95 text-center touch-manipulation whitespace-normal min-h-[52px]"
          >
