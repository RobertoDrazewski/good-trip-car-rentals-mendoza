  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-[1000] transition-all duration-500 ${
      isScrolled 
        ...
        : 'h-36 bg-transparent' // Aumentamos h-32 a h-36 para dar más espacio inicial
    }`}>
      ...
      <div className="max-w-[1440px] h-full mx-auto px-6 md:px-12 flex justify-between items-center relative">
          ...
          <a href="/" className="inline-block transition-transform hover:scale-105 active:scale-95">
            <img 
              ...
              className={`transition-all duration-500 object-contain drop-shadow-2xl ${
                isScrolled 
                  ? 'h-20 translate-y-0' // Al hacer scroll vuelve a su posición normal
                  : 'h-48 translate-y-6' // +20% de tamaño (h-48) y bajado 24px (translate-y-6)
              }`}
            />
          </a>
        </div>
        ...
        <div className="flex-1 flex justify-end items-center gap-6">
          <div className={`flex items-center gap-2 rounded-full h-12 px-5 border transition-all ${
            isScrolled 
            ...
            <select 
              ...
              value={i18n.language}
              className="bg-transparent text-[11px] text-white outline-none cursor-pointer font-black uppercase tracking-widest border-none focus:ring-0"
            >
            ...
            <div className="flex flex-col gap-8">
              {navLinks.map(link => (
                <a 
                  ...
                  className="text-2xl font-black uppercase text-white hover:text-yellow-500"
                >
                  {link.label}
                </a>
              ))}
