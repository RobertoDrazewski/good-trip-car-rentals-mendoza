...
export default function AdminDashboard() {
  ...
  // UX Feedback para el despacho de correos en Resend
  const [isInviting, setIsInviting] = useState(false);

  // CAPTURA DINÁMICA DEL NOMBRE DE USUARIO LOGUEADO
  const usuarioLogueado = localStorage.getItem('userName') || 'Admin';

  // ESTADOS DE ALERTA DE LEADS EN VERDE ESMERALDA
  ...
  // REPRODUCCIÓN AUDIO ALERTA POR WEB AUDIO API
  ...
  const obtenerUrlEnrutamientoDinamico = (ruta) => {
    if (!ruta) return "http://maps.google.com/0";
  ...
  if (loading) return <div className="h-screen w-full flex flex-col items-center justify-center bg-slate-900 gap-6"><Activity className="text-sky-500 animate-pulse" size={64} /><span className="text-sky-500 font-black tracking-[0.3em] italic text-xl uppercase">Good Trip Pro...</span></div>;
    ...
    <div className="flex min-h-screen w-full bg-[#f8fafc] font-sans text-slate-800 max-lg:flex-col relative">
      ...
      <aside className="w-80 bg-slate-800 p-6 flex flex-col sticky top-0 h-screen border-r border-slate-700 z-40 max-lg:w-full max-lg:h-auto max-lg:p-5 max-lg:border-b">
        ...
        <div className="mb-6 max-lg:mb-3">
          <button 
            ...
            className="w-full flex items-center gap-4 px-6 py-3 bg-slate-700 hover:bg-slate-600 text-sky-400 rounded-xl transition-all font-black text-[10px] uppercase tracking-wider shadow-inner cursor-pointer"
          >
        ...
        {/* LOGOUT INTEGRADO AL SIDEBAR COHERENTE */}
        <div className="mt-auto pt-4 border-t border-slate-700 max-lg:hidden">
          <button 
            onClick={handleLogout}
            className="w-full flex items-center justify-between px-5 py-3.5 bg-slate-700/40 hover:bg-rose-500/10 text-slate-400 hover:text-rose-400 rounded-xl transition-all font-black text-[10px] uppercase tracking-widest cursor-pointer"
          >
            <div className="flex items-center gap-3">
      ...
      <main className="flex-1 p-6 md:p-10 overflow-y-auto w-full">
        ...
        <header className="flex justify-between items-center mb-8 md:mb-10 max-sm:flex-col max-sm:items-start max-sm:gap-4">
          <div>
            <h1 className="text-2xl md:text-3xl font-black text-slate-800 uppercase italic tracking-tighter">Panel de <span className="text-sky-500">Control</span></h1>
            <p className="text-slate-400 text-[9px] md:text-[10px] font-black uppercase tracking-[0.2em] mt-1 italic">Sistema Operativo • Good Trip Car Rentals</p>
          </div>
          
          {/* Topbar Perfil Móvil */}
          <div className="flex items-center gap-4 bg-white p-2 pr-6 rounded-2xl border border-slate-200 shadow-sm sm:border-slate-100 max-sm:w-full max-sm:justify-between lg:hidden">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 bg-slate-800 rounded-xl flex items-center justify-center text-sky-400 font-black shadow-md">
                {usuarioLogueado.charAt(0)}
              </div>
              <div className="flex flex-col text-left">
                <span className="text-sm font-black text-slate-800">{usuarioLogueado}</span>
                <span className="text-[9px] font-black text-sky-600 uppercase tracking-tighter bg-sky-50 px-2 py-0.5 rounded-md w-fit">Admin Activo</span>
              </div>
            </div>
            <button onClick={handleLogout} className="p-2 text-slate-300 hover:text-rose-500 transition-colors bg-slate-50 rounded-lg"><LogOut size={16} /></button>
          </div>
        </header>
          ...
          return (
            <div className="space-y-6 md:space-y-10 animate-in fade-in duration-500">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
                ...
                />
                <div className="bg-slate-800 p-6 rounded-[2rem] text-white flex flex-col justify-center shadow-sm border border-slate-700">
                  <p className="text-[10px] font-black uppercase text-sky-400 tracking-[0.2em] mb-1">Dólar Configurado</p>
              ...
              <div className="bg-white rounded-[1.5rem] md:rounded-[2.5rem] shadow-xl border border-slate-100 overflow-hidden">
                <div className="bg-slate-800 px-6 md:px-8 py-4 flex items-center justify-between border-b border-slate-700">
                  <p className="text-[11px] md:text-xs font-black text-sky-400 uppercase tracking-widest italic">📋 Listado Maestro Sincronizado</p>
                  <span className="text-[9px] bg-slate-700 text-slate-300 px-3 py-1 rounded-full font-bold">{reservasCronologicas.length} Ops</span>
                </div>
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {reservasCronologicas.map(r => (
                        <tr key={r.id} className="hover:bg-slate-50/50 transition-colors">
                          <td className="p-6">
                            <select value={r.estado || 'pendiente'} onChange={(e) => handleAction('post', `${apiUrl}/api/admin/cambiar-estado`, {id: r.id, estado: e.target.value})} className="bg-sky-50 text-sky-700 font-black text-[10px] px-3 py-2 uppercase rounded-xl border-none outline-none shadow-sm cursor-pointer">
                          ...
                          <td className="p-6">
                            <select value={localStorage.getItem(`g_res_${r.id}`) || 'pendiente'} onChange={(e) => { localStorage.setItem(`g_res_${r.id}`, e.target.value); fetchData(); }} className="bg-slate-100 font-black text-[10px] px-3 py-2 uppercase rounded-xl border-none outline-none cursor-pointer">
                              <option value="pendiente">⏳ Pendiente</option>
                          ...
                          <td className="p-6 text-left">
                            <p className="text-base font-black italic uppercase text-slate-800 leading-none">{r.cliente_nombre}</p>
                            <div className="flex items-center gap-2 mt-1">
                              <span className="text-[10px] font-black text-sky-600 uppercase tracking-widest">{r.auto_modelo || 'Vehículo'}</span>
                              <span className="text-[9px] font-black bg-slate-800 text-sky-400 px-1.5 py-0.5 rounded">{r.patente || 'S/P'}</span>
                            </div>
                            <p className="text-xs font-black text-emerald-600 mt-1">Monto: ${parseFloat(r.monto_total_ars || 0).toLocaleString('es-AR')}</p>
                          </td>
                          <td className="p-6 text-center">
                            {String(r.sillita) === '1' || String(r.sillita) === 'true' ? <span className="bg-emerald-50 text-emerald-700 font-black text-[10px] px-2.5 py-1 rounded-full border border-emerald-100">✔️ Sí</span> : <span className="bg-slate-50 text-slate-400 font-bold text-[10px] px-2.5 py-1 rounded-full">❌ No</span>}
                          </td>
                          <td className="p-6 text-left">
                            <p className="text-xs font-black text-slate-600">{r.fecha_inicio?.substring(0, 10)}</p>
                            <p className="text-xs font-black text-slate-800 italic mt-0.5">⏱ {r.hora_inicio || '10:00'}</p>
                            <p className="text-[10px] font-black text-sky-600 uppercase mt-1 italic">📍 {r.lugar_retiro}</p>
                          </td>
                          <td className="p-6 text-left">
                            <p className="text-xs font-black text-slate-600">{r.fecha_fin?.substring(0, 10)}</p>
                            <p className="text-xs font-black text-slate-800 italic mt-0.5">⏱ {r.hora_fin || '10:00'}</p>
                            <p className="text-[10px] font-black text-slate-500 uppercase mt-1 italic">📍 {r.lugar_devolucion}</p>
                          </td>
                          <td className="p-6 text-center">
                            <a href={`https://wa.me/${String(r.cliente_whatsapp || '').replace(/[^0-9]/g, '')}`} target="_blank" rel="noreferrer" className="inline-flex p-2.5 bg-slate-100 text-slate-600 rounded-xl hover:bg-slate-800 hover:text-white transition-all"><MessageCircle size={16} /></a>
                          </td>
                          <td className="p-6 text-center">
                            <div className="flex items-center justify-center gap-2">
                              ...
                              <button onClick={() => descargarTicketPresupuesto(r)} className="p-2.5 bg-slate-800 text-sky-400 rounded-xl hover:bg-sky-500 hover:text-white transition-all cursor-pointer"><FileText size={16} /></button>
                              <button onClick={() => { if(window.confirm('⚠️ ¿Eliminar reserva?')) handleAction('delete', `${apiUrl}/api/admin/reservas/${r.id}`); }} className="p-2.5 bg-slate-50 text-slate-400 rounded-xl hover:bg-rose-500 hover:text-white transition-all cursor-pointer"><Trash2 size={16} /></button>
                            </div>
                          </td>
                        </tr>
                      ))}
                  ...
                  {reservasCronologicas.map(r => (
                    <div key={r.id} className="p-5 space-y-4 bg-white shadow-sm mb-3 rounded-xl border border-slate-100 mx-3 my-3">
                      ...
                      <div className="text-left">
                        <h4 className="text-lg font-black uppercase italic tracking-tight text-slate-800">{r.cliente_nombre}</h4>
                        <p className="text-xs font-bold text-sky-600 uppercase mt-0.5">{r.auto_modelo || 'Vehículo de Flota'}</p>
                        <p className="text-sm font-black text-emerald-600 mt-1">Monto: ${parseFloat(r.monto_total_ars || 0).toLocaleString('es-AR')}</p>
                      </div>

                      <div className="grid grid-cols-2 gap-3 bg-slate-50 p-3 rounded-xl border border-slate-100 text-[11px] text-left">
                        <div>
                          <p className="font-black text-slate-400 uppercase tracking-tighter">🛫 RETIRO</p>
                          <p className="font-bold text-slate-700 mt-0.5">{r.fecha_inicio?.substring(0, 10)} • {r.hora_inicio?.substring(0,5)} hs</p>
                        ...
                        <div>
                          ...
                          <p className="font-bold text-slate-700 mt-0.5">{r.fecha_fin?.substring(0, 10)} • {r.hora_fin?.substring(0,5)} hs</p>
                      ...
                      <div className="flex justify-between items-center pt-2 border-t border-slate-100">
                        <div className="flex items-center gap-2">
                          ...
                          <select value={localStorage.getItem(`g_res_${r.id}`) || 'pendiente'} onChange={(e) => { localStorage.setItem(`g_res_${r.id}`, e.target.value); fetchData(); }} className="bg-slate-100 font-black text-[9px] px-2 py-1 uppercase rounded-md border-none outline-none">
                            <option value="pendiente">⏳ Pendiente</option>
                        ...
                        <div className="flex gap-1.5">
                          ...
                          <button onClick={() => descargarTicketPresupuesto(r)} className="p-2 bg-slate-800 text-sky-400 rounded-lg"><FileText size={16} /></button>
                          <button onClick={() => { if(window.confirm('⚠️ ¿Eliminar?')) handleAction('delete', `${apiUrl}/api/admin/reservas/${r.id}`); }} className="p-2 bg-slate-50 text-slate-300 rounded-lg hover:text-rose-500"><Trash2 size={16} /></button>
                        </div>
                      </div>
                    </div>
                  ))}
        ...
        {activeTab === 'calendario' && (
          <div className="space-y-6 animate-in fade-in duration-500">
            <div className="bg-white p-4 md:p-10 rounded-[1.5rem] md:rounded-[2.5rem] shadow-xl border border-slate-100 overflow-hidden">
              <h3 className="font-black uppercase italic text-lg md:text-xl text-slate-800 tracking-wider mb-6 text-left">Ocupación en Tiempo Real</h3>
              ...
              <style>{`
                ...
                .react-calendar__tile--active { background-color: #f0f9ff !important; border: 2px solid #0ea5e9 !important; color: #0369a1 !important; }
                @media (max-width: 640px) { .react-calendar__tile { height: 80px !important; } }
                .patente-tag { font-size: 7px !important; font-weight: 900 !important; text-transform: uppercase; background-color: #1e293b !important; color: #0ea5e9 !important; padding: 1px 2px !important; border-radius: 3px !important; width: 100% !important; text-align: center; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
            ...
            <div className="bg-white p-6 md:p-8 rounded-[1.5rem] md:rounded-[2.5rem] shadow-xl border border-slate-100 text-left">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-slate-100 pb-4 mb-6 gap-2">
                <div>
                  <h4 className="font-black uppercase italic text-md md:text-lg text-slate-800 tracking-wider">Estado de la Flota</h4>
                  <p className="text-xs text-slate-500 font-medium">Monitoreando disponibilidad para el día: <span className="text-sky-600 font-bold">{selectedDate.toLocaleDateString('es-AR', { day: 'numeric', month: 'long', year: 'numeric' })}</span></p>
                </div>
                    ...
                    <div key={auto.id} className={`flex items-center p-4 rounded-2xl border transition-all duration-300 ${estaOcupado ? 'border-rose-100 bg-rose-50/30' : 'border-slate-100 bg-slate-50/50 hover:border-emerald-200 hover:bg-emerald-50/10'}`}>
                      ...
                      <div className="ml-4 flex-grow flex flex-col justify-between h-full min-w-0">
                        <div className="min-w-0">
                          <h5 className="font-bold text-slate-800 text-sm truncate uppercase tracking-tight">{auto.modelo}</h5>
                        ...
                        <div className="flex items-center gap-2 mt-2">
                          {estaOcupado ? (
                            <>
                              ...
                              <span className="text-[10px] font-black uppercase text-rose-600 truncate">Ocupado: {reservaActiva.cliente_nombre}</span>
                            </>
                          ) : (
                            <>
                              ...
                              <span className="text-[10px] font-black uppercase text-emerald-600">Disponible</span>
                            </>
                          )}
                        </div>
                      </div>
          ...
          <div className="space-y-6 animate-in fade-in duration-500 text-left">
             <div className="bg-white p-6 md:p-10 rounded-[1.5rem] md:rounded-[2.5rem] shadow-lg border border-slate-100">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-4">
                  ...
                  }} className="w-full bg-slate-800 text-sky-400 p-4 rounded-2xl font-black flex items-center justify-center gap-2 hover:bg-sky-500 hover:text-white transition-all uppercase cursor-pointer"><Sparkles size={18}/> IA Optimizer</button>
                  <button onClick={async () => {
                    const authHeader = getAuthConfig();
                  ...
                  }} className="w-full bg-sky-600 text-white p-4 rounded-2xl font-black uppercase italic cursor-pointer">Guardar Ruta</button>
                </div>
              </div>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">{rutas.map(r => (
              <div key={r.id} className="bg-white p-5 rounded-2xl flex justify-between items-center shadow-sm border border-slate-100">
                ...
                <button onClick={()=>handleAction('delete', `${apiUrl}/api/routes/${r.id}`)} className="text-slate-300 hover:text-rose-500 cursor-pointer"><Trash2 size={18}/></button>
              </div>
            ))}</div>
          </div>
        )}
        ...
        {activeTab === 'mapa' && (
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 h-auto lg:h-[700px] animate-in fade-in duration-500 text-left">
            <div className="bg-slate-800 p-6 rounded-[1.5rem] md:rounded-[2.5rem] border border-slate-700 flex flex-col justify-between gap-5">
              <div className="space-y-4 overflow-y-auto max-h-[400px] lg:max-h-[520px]">
                <div>
                  <p className="text-[9px] font-black uppercase text-sky-400 tracking-[0.2em] mb-1">Logística de Entregas VIP</p>
                ...
                <div className="bg-slate-700 p-2 rounded-xl border border-slate-600">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <button onClick={() => setPuntoOrigenMapa('aeropuerto')} className={`py-2 px-1 rounded-lg text-[9px] font-black uppercase transition-all ${puntoOrigenMapa === 'aeropuerto' ? 'bg-sky-500 text-white' : 'bg-slate-600 text-slate-300'}`}>✈️ Aeropuerto</button>
                    <button onClick={() => setPuntoOrigenMapa('km0')} className={`py-2 px-1 rounded-lg text-[9px] font-black uppercase transition-all ${puntoOrigenMapa === 'km0' ? 'bg-sky-500 text-white' : 'bg-slate-600 text-slate-300'}`}>🏢 Km Cero</button>
                  </div>
                </div>
                <div className="w-full border-t border-slate-700 pt-3 flex flex-col gap-2">
                  <p className="text-[9px] font-black uppercase text-slate-400 mb-1">⛰️ Destino:</p>
                  {rutas.map((r, idx) => (
                    <button key={r.id || idx} onClick={() => setRutaMapaSeleccionada(r)} className={`w-full text-left p-2.5 rounded-xl text-[10px] font-black uppercase truncate ${rutaMapaSeleccionada?.id === r.id ? 'bg-sky-500 text-white' : 'bg-slate-700 text-slate-300'}`}>👉 {r.titulo}</button>
                  ))}
                </div>
              </div>
            </div>
        ...
        {activeTab === 'flota' && (
          <div className="space-y-6 animate-in fade-in duration-500 text-left">
            <button onClick={() => setShowAddAuto(!showAddAuto)} className="w-full md:w-auto bg-slate-800 text-white px-8 py-4 rounded-xl font-black uppercase italic hover:bg-sky-500 hover:text-white transition-all shadow-md cursor-pointer">{showAddAuto ? '❌ Cancelar' : '➕ Agregar Unidad'}</button>
            {showAddAuto && (
              <div className="bg-white p-6 md:p-12 rounded-[1.5rem] md:rounded-[3.5rem] shadow-2xl border border-slate-100 grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-4">
                  ...
                  }} className="w-full bg-slate-800 text-white p-4 rounded-xl font-black text-md tracking-widest hover:bg-sky-500 hover:text-white transition-all uppercase">🚀 GUARDAR VEHÍCULO</button>
                </div>
              </div>
            )}
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">{autos.map(a => (
              <div key={a.id} className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm relative group hover:shadow-md transition-all">
                <button onClick={() => handleAction('delete', `${apiUrl}/api/admin/autos/${a.id}`)} className="absolute top-4 right-4 text-slate-300 hover:text-rose-500 p-1"><Trash2 size={18}/></button>
                <img src={a.imagen_url ? `${apiUrl}${a.imagen_url}` : '/car-placeholder.jpg'} className="w-full h-36 object-contain mb-4" alt={a.modelo} />
                <div className="flex items-center gap-2 mb-2"><div className="w-3 h-3 rounded-full border" style={{backgroundColor: a.color}} /><h4 className="text-md font-black uppercase italic text-slate-800">{a.modelo}</h4></div>
                <div className="flex justify-between border-t border-slate-50 pt-3 text-[9px] font-black text-slate-400 uppercase tracking-widest"><span>{a.patente}</span><span className="text-sky-500 font-mono">USD {a.precio_base_usd}/DÍA</span></div>
              </div>
            ))}</div>
          </div>
        )}
        ...
        {activeTab === 'precios' && (() => {
          ...
          return (
            <div className="space-y-6 max-w-5xl mx-auto animate-in fade-in duration-500 text-left">
              <div className="bg-slate-800 p-6 md:p-8 rounded-2xl text-white flex flex-col lg:flex-row justify-between items-center gap-4 shadow-md border border-slate-700">
                <div className="text-center lg:text-left">
                  <p className="text-[9px] font-black uppercase text-sky-400 tracking-wider">Tarifas del Sistema</p>
            ...
            <div className="bg-white p-6 md:p-12 rounded-2xl shadow-md border border-slate-100">
              ...
              <div className="space-y-4">
                ...
                <button 
                  ...
                  className="w-full bg-slate-800 text-sky-400 p-4 rounded-xl font-black text-sm flex items-center justify-center gap-2 uppercase tracking-wider hover:bg-sky-500 hover:text-white transition-all cursor-pointer disabled:bg-slate-200"
                >
                  {isInviting ? (
            ...
            <div className="space-y-3">
              ...
              {admins.map(adm => (
                <div key={adm.id} className="bg-white p-4 rounded-xl flex justify-between items-center shadow-sm border border-slate-100">
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 bg-slate-100 rounded-lg flex items-center justify-center font-black text-slate-500 uppercase text-xs">
                      {adm.nombre?.charAt(0)}
                    </div>
                    <div>
                      <p className="font-black text-slate-800 uppercase italic text-xs">{adm.nombre}</p>
                      <p className="text-[8px] font-mono text-slate-400 lowercase">{adm.email}</p>
                    </div>
                  </div>
                  <button onClick={() => handleAction('delete', `${apiUrl}/api/admin/users/${adm.id}`)} className="text-slate-300 hover:text-rose-500 cursor-pointer transition-colors"><Trash2 size={16}/></button>
                </div>
              ))}
            </div>
          </div>
...
function StatCard({label, val, icon}) {
  return <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm hover:shadow-md transition-all group text-left"><div className="bg-slate-50 w-12 h-12 rounded-xl flex items-center justify-center mb-4 text-xl group-hover:bg-sky-50 transition-colors">{icon}</div><p className="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-0.5">{label}</p><p className="text-2xl font-black text-slate-800 italic tracking-tighter leading-none">{val}</p></div>;
}
...
function PriceBox({ label, val, onSave, icon }) {
  ...
  return (
    <div className="bg-slate-50/70 p-3 sm:p-4 rounded-xl border border-slate-100 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 hover:bg-slate-100/50 transition-colors text-left">
      ...
      <div className="flex items-center gap-2 flex-1 justify-end w-full sm:w-auto">
        <input type="number" value={v} onChange={e => setV(e.target.value)} className="w-full sm:w-36 text-right text-md font-black text-slate-800 bg-white border-2 border-slate-200 focus:border-sky-500 rounded-lg px-3 py-1.5 outline-none font-mono" placeholder="0" />
        <button onClick={handleLocalSave} className={`px-4 py-2 rounded-lg font-black uppercase text-[9px] tracking-wider transition-all min-w-[80px] text-center ${guardadoOk ? 'bg-emerald-600 text-white' : 'bg-slate-800 text-sky-400 hover:bg-sky-500 hover:text-white'}`}>{guardadoOk ? "✔️ Ok" : "Guardar"}</button>
      </div>
    </div>
  );
}

const inputStyle = "w-full p-3.5 bg-slate-50 rounded-xl border-2 border-slate-100 font-black text-slate-800 text-sm outline-none focus:border-sky-500 transition-colors text-left";